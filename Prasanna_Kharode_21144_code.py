...
# Selection of regression techniques  
    ...
    # Random Forest 
            ...
            'rgr__n_estimators':(200,250300,350,400),
            'rgr__max_depth':(200,300,350),
...
# Load the data 
    ...
    # Load training data and target labels
          ...
          target_labels = pd.read_csv('training_data_targets.csv', names=['target'])

    # Preprocessing steps
    # One-Hot Encoding for categorical variable 'ADDRESS'
          encoded_training_data = pd.get_dummies(training_data, columns=['ADDRESS'])

    # Filtering outliers based on the 'labels' column <= 200
          data_with_labels = pd.concat([encoded_training_data, target_labels], axis=1)
...
rgr=regression(' ', rgr_opt='dt',no_of_selected_features=800)
